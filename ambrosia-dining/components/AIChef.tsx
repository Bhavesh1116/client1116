...
import { Send, Loader2, MapPin, ChefHat, X } from 'lucide-react';
...
export const AIChef: React.FC<AIChefProps> = ({ onRecommend, fullMenu }) => {
  ...
  return (
    <div className="fixed bottom-6 right-6 z-50 flex flex-col items-end pointer-events-none">
      ...
      {/* Chat Window */}
      {isOpen && (
        <div className="pointer-events-auto mb-4 w-80 md:w-96 bg-dark-800/95 backdrop-blur-xl border border-gold-500/30 rounded-2xl shadow-[0_0_50px_rgba(0,0,0,0.5)] overflow-hidden animate-reveal flex flex-col max-h-[500px]">
          
          {/* Header */}
          <div className="bg-gradient-to-r from-gold-600 to-gold-500 p-4 flex justify-between items-center">
            <div className="flex items-center gap-2 text-dark-950 font-bold">
              <ChefHat className="w-6 h-6" />
              <h3 className="font-serif">Chef's Assistant</h3>
            </div>
            {/* Mode Toggles */}
          ...
          <div className="p-4 flex-1 overflow-y-auto space-y-4 min-h-[300px]">
            ...
            {loading && (
              <div className="flex justify-start">
                 <div className="bg-dark-700 rounded-2xl rounded-tl-none p-3 border border-white/5">
      ...
      >
        <span className="font-bold font-serif hidden md:block group-hover:block">
          {isOpen ? 'Close Assistant' : 'Ask the Chef'}
        </span>
