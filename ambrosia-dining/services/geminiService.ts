...
export const askRestaurantAgent = async (question: string) => {
  if (!apiKey) return "I can't connect to live maps right now, but we are located at " + RESTAURANT_INFO.address;
...
export const getLiveStatus = async () => {
   if (!apiKey) return { text: "Live satellite feed unavailable. Expect the usual line!", source: null };
